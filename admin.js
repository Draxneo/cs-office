...
// US formatting helpers. fmtDate -> M/D/YYYY ; fmtDateTime -> M/D/YYYY h:mm AM/PM (local) ; fmtPhone -> (210) 555-1234
function fmtDate(s) { if (!s) return ""; const d = new Date(String(s).length <= 10 ? String(s) + "T00:00:00" : s); if (isNaN(+d)) return String(s); return (d.getMonth() + 1) + "/" + d.getDate() + "/" + d.getFullYear(); }
function fmtDateTime(s) { if (!s) return ""; const d = new Date(s); if (isNaN(+d)) return String(s); let h = d.getHours(); const ap = h < 12 ? "AM" : "PM"; h = h % 12 || 12; const m = d.getMinutes(); return (d.getMonth() + 1) + "/" + d.getDate() + "/" + d.getFullYear() + " " + h + ":" + (m < 10 ? "0" + m : m) + " " + ap; }
// (fmtPhone already defined below — reuse it.)
...
function todoRow(jid, s) {
  ...
  return `<div style="border-top:1px solid var(--line)">
      <div style="display:flex;align-items:center;gap:8px;padding:7px 0">
        <span style="width:9px;height:9px;border-radius:50%;background:${instStColor(s.status)};flex:none"></span>
        <span style="flex:1">${esc(s.label)}${s.due_date ? ` <span class="muted" style="font-size:11px">due ${esc(fmtDate(s.due_date))}</span>` : ""}${s.overdue ? ' <span style="color:#f87171">&#9888;</span>' : ""}</span>
...
function custRenderList(list, header, showTouch) {
  ...
  r.innerHTML = head + list.map((c) => `<div class="card" data-cust="${esc(c.id)}" style="cursor:pointer;padding:9px 12px;margin-bottom:7px;display:flex;justify-content:space-between;align-items:center;gap:8px">
      <div><strong>${esc(c.name || "(no name)")}</strong> <span class="muted" style="font-size:12px">&middot; ${esc(c.phone ? fmtPhone(c.phone) : "")}${c.jurisdiction ? " &middot; " + esc(c.jurisdiction) : ""}</span></div>
      ${showTouch && c.last_touch ? `<span class="muted" style="font-size:11px;white-space:nowrap">${esc(c.via || "")} &middot; ${esc(fmtDate(c.last_touch))}</span>` : ""}
    </div>`).join("");
...
function custRenderJobs(jobs, label) {
  ...
  r.innerHTML = head + jobs.map((j) => `<div class="card" style="padding:9px 12px;margin-bottom:7px">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap">
        <div><strong data-cust="${esc(j.customer_id || "")}" style="cursor:pointer;text-decoration:underline">${esc(j.customer || "(unknown)")}</strong> ${j.install ? custBadge("Install", "rgba(52,211,238,.15)") : ""} <span class="muted" style="font-size:12px">&middot; Job ${esc(j.job || "?")} &middot; ${esc(fmtDate(j.date))}${j.total != null ? " &middot; $" + j.total.toLocaleString() : ""}</span></div>
...
async function custOpen(id) {
  ...
  // photos come pre-merged from the backend (texted-in + Housecall job/estimate attachments, dated)
  d._photos = d.photos || [];
  ...
  return `<button class="btn" id="cust-back" style="margin-bottom:10px">&#8592; Back</button>
    <div class="card" style="padding:14px">
      ...
      <div class="muted" style="font-size:13px;line-height:1.8">
        &#128222; ${esc(c.phone ? fmtPhone(c.phone) : "—")}${c.phone_type ? " (" + esc(c.phone_type) + ")" : ""} &nbsp;&nbsp; &#9993;&#65039; ${esc(c.email || "—")}<br>
        &#127968; ${esc(c.address || "—")}<br>
    ...
    <div style="display:flex;flex-wrap:wrap;gap:2px;border-bottom:1px solid var(--line);margin:6px 0 12px;overflow-x:auto">
      ${tab("overview", "Overview")}${tab("equipment", "Equipment", (d.counts && d.counts.equipment) || 0)}${tab("jobs", "Jobs", d.counts.jobs)}${tab("estimates", "Estimates", (d.counts && d.counts.estimates) || 0)}${tab("calls", "Calls", d.counts.calls)}${tab("texts", "Texts", d.counts.texts)}${tab("photos", "Photos", d._photos.length)}${tab("todos", "To-Dos", (d.installs || []).length)}
    </div>
...
function custTabHtml(key, d) {
  ...
  return `<div class="card" style="font-size:13px;line-height:1.7">
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px">${link("equipment", "Equipment " + ((d.counts && d.counts.equipment) || 0))} ${link("jobs", "Jobs " + d.counts.jobs)} ${link("estimates", "Estimates " + ((d.counts && d.counts.estimates) || 0))} ${link("calls", "Calls " + d.counts.calls)} ${link("texts", "Texts " + d.counts.texts)} ${link("photos", "Photos " + d._photos.length)} ${link("todos", "To-Dos " + (d.installs || []).length)}</div>
    ${lastJob ? `<div><b>Last job:</b> ${esc(lastJob.job || "—")}${lastJob.install ? " (install)" : ""} &middot; ${esc(fmtDate(lastJob.date))} &middot; ${esc(lastJob.status || "")} &mdash; <span class="muted">${esc(lastJob.description || "")}</span></div>` : `<div class="muted">No jobs yet.</div>`}
...
function ensureCalStyles() {
  ...
  s.textContent =
    ".ic-rail{position:fixed;top:0;left:0;height:100vh;width:300px;max-width:85vw;background:var(--surface);border-right:1px solid var(--line);box-shadow:2px 0 18px rgba(0,0,0,.35);transform:translateX(-100%);transition:transform .22s ease;z-index:60;display:flex;flex-direction:column}" +
    ".ic-rail.open{transform:translateX(0)}" +
...
async function renderInstallCalendar() {
  ...
  main.innerHTML = `<h2 class="sec">Install Calendar &#128197;</h2>
    <p class="sub">Crews are rows; hours run across the top. Drag a job from the <b>Unscheduled</b> drawer onto a crew's row at the time you want, or drag a block to a new row/time to move it. Hover for details.</p>
  ...
  $("ic-board").querySelectorAll(".ic-ev").forEach((el) => {
    ...
    el.addEventListener("mouseenter", () => {
      tip.innerHTML = `<b>${esc(a.customer_name || "?")}</b>${a.job_number ? ` <span class="muted">#${esc(a.job_number)}</span>` : ""}<br><span class="muted">${esc(timeShort(a.scheduled_start))} &middot; &#128119; ${esc(a.installer || "?")}</span>${a.description ? `<br>${esc(a.description)}` : ""}${a.note ? `<br><i>${esc(a.note)}</i>` : ""}<br><a href="#" id="ic-unassign" style="color:#f87171">Unassign</a>`;
      const r = el.getBoundingClientRect();
      tip.style.left = Math.min(r.left, window.innerWidth - 290) + "px"; tip.style.top = (r.bottom + 6) + "px"; tip.style.display = "block";
      const un = $("ic-unassign"); if (un) un.onclick = (ev) => { ev.preventDefault(); hideTip(); if (confirm("Unassign this install? It goes back to the Unscheduled drawer.")) idApi("unassign", { assignment_id: a.id }).then(() => renderInstallCalendar()); };
    });
    el.addEventListener("mouseleave", () => { setTimeout(() => { if (!tip.matches(":hover")) hideTip(); }, 140); });
  });
  ...
  rail.querySelectorAll(".ic-railcard").forEach((el) => {
    const p = installs[el.getAttribute("data-pool")];
    el.addEventListener("dragstart", () => { icDrag = { type: "new", job: p }; hideTip(); });
    el.addEventListener("dragend", () => { icDrag = null; });
  });
}
