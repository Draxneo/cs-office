...
function custTabHtml(key, d) {
  ...
  if (key === "photos") return d._photos.length ? `<div style="display:flex;flex-wrap:wrap;gap:10px">${d._photos.map((p) => `<a href="${esc(p.url || p.u)}" target="_blank" rel="noopener" style="text-decoration:none"><div style="width:140px"><img src="${esc(p.url || p.u)}" loading="lazy" style="width:140px;height:140px;object-fit:cover;border-radius:8px;display:block;background:var(--surface2)"/><div class="muted" style="font-size:10px;margin-top:2px">${esc(p.source || "")}${p.date ? " &middot; " + esc(p.date) + (p.approx ? "~" : "") : ""}</div></div></a>`).join("")}</div>` : `<div class="muted">No photos.</div>`;
